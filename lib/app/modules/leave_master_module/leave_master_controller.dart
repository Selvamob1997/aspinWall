...
class LeaveMasterController extends GetxController{
  ...
  getLeave(){
    Allapi.insertLeaveMaster(1, sessionUseId, leaveType.text, leaveCode, fromdate.text,
      ...
      if(value.statusCode==200){
        ...
        ELLeave = double.parse(jsonDecode(value.body)['result'][0]['EmerLeave'].toString()) ,
        SLLeave = double.parse(jsonDecode(value.body)['result'][0]['SeekLeave'].toString()),
        Comff = double.parse(jsonDecode(value.body)['result'][0]['CompOff'].toString()),
        },
        update(),
        Utilities.closeLoader(),
        if(getDocNo==0){
